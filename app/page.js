...
export default function LiffPage() {
  ...
  const [sysSettings, setSysSettings] = useState({ filmCount: 0, isPaused: false, autoCleanupDays: 3, isCleanupEnabled: false });
  ...
  const UserPointsTab = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-[#0f172a] rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden border border-slate-700">
        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-600 rounded-full mix-blend-multiply filter blur-3xl opacity-50"></div>
  ...
  const UserPhotosTab = () => (
    <div className="space-y-4 pb-32 animate-in fade-in duration-500">
      ...
      {data.logs.length > 0 ? data.logs.map((log) => (
        <a key={log.id} href={`https://drive.google.com/file/d/${log.driveFileId}/view`} target="_blank" rel="noopener noreferrer" className="block bg-white rounded-[2rem] p-3 flex items-center gap-4 shadow-sm border border-slate-100 active:scale-[0.98] group">
          <div className="relative w-20 h-20 shrink-0">
            <img src={`https://drive.google.com/thumbnail?id=${log.driveFileId}&sz=w200`} referrerPolicy="no-referrer" className="w-full h-full rounded-2xl object-cover bg-slate-50" />
            <div className="absolute inset-0 bg-black/20 rounded-2xl opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"><span className="text-white text-xs">↗</span></div>
          </div>
          <div className="flex-1 py-1">
            <div className="flex justify-between items-start mb-1">
              <span className="text-xs font-bold text-slate-400">#{log.id.slice(-4)}</span>
              <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase ${log.status === '已完成' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{log.status}</span>
            </div>
    ...
    <div className="text-center space-y-6 animate-in zoom-in-95 duration-500">
      <div className="bg-white rounded-[3rem] p-10 shadow-xl border border-slate-100 relative overflow-hidden">
        ...
        <div className="relative">
          <img src={user?.pictureUrl} className="w-24 h-24 rounded-full border-4 border-white shadow-md mx-auto mb-4" />
      ...
      {adminPhotos.map(photo => (
        <div key={photo.id} className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-slate-100">
          <a href={`https://drive.google.com/file/d/${photo.driveFileId}/view`} target="_blank" rel="noopener noreferrer" className="relative block">
            ...
            <div className="absolute top-3 right-3 bg-black/50 backdrop-blur-md text-white text-[10px] px-3 py-1 rounded-full font-bold">點擊檢視 ↗</div>
          </a>
  ...
  const AdminMemberTab = () => (
    <div className="space-y-6 text-center pb-32">
      ...
      {!scannedUser ? (
        <button onClick={async () => { try { const res = await liff.scanCodeV2(); const userRes = await fetch(`/api/admin?type=member&userId=${res.value}`); if (userRes.ok) setScannedUser(await userRes.json()); } catch(e) {} }} className="w-full bg-blue-600 text-white py-14 rounded-[3rem] text-2xl font-black shadow-xl shadow-blue-200 active:scale-[0.98] transition-all flex flex-col items-center gap-2">
          <span className="text-4xl">📷</span><span>啟動相機掃描</span>
        </button>
      ) : (
  ...
  const AdminSettingsTab = () => (
    <div className="space-y-6 pb-32 animate-in fade-in">
      ...
      <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 space-y-8">
        ...
        {/* 底片數量設定 */}
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-2">相印機剩餘底片</label>
          <div className="flex gap-4">
            <input type="number" value={sysSettings.filmCount} onChange={(e) => setSysSettings({...sysSettings, filmCount: Number(e.target.value)})} className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-xl font-black outline-none focus:border-blue-500" />
            <button onClick={() => updateSettings(sysSettings)} className="bg-blue-600 text-white px-8 rounded-2xl font-bold shadow-lg active:scale-95">儲存</button>
          </div>
          <p className="text-[10px] text-slate-400 mt-2">每次學生上傳照片，此數量會自動扣除。歸零時將阻擋上傳。</p>
        </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            <div>
              <h3 className="font-bold text-slate-800">定期清理雲端文件</h3>
              <p className="text-[10px] text-slate-400">自動刪除過舊照片以節省 Drive 空間</p>
            </div>
            <button onClick={() => updateSettings({...sysSettings, isCleanupEnabled: !sysSettings.isCleanupEnabled})} className={`w-14 h-7 rounded-full relative transition-colors ${sysSettings.isCleanupEnabled ? 'bg-emerald-500' : 'bg-slate-200'}`}>
              <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all ${sysSettings.isCleanupEnabled ? 'left-8' : 'left-1'}`}></div>
            </button>
          </div>
          {sysSettings.isCleanupEnabled && (
            <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <span className="text-xs font-bold text-slate-500">保留天數:</span>
              <input type="range" min="1" max="14" value={sysSettings.autoCleanupDays} onChange={(e) => updateSettings({...sysSettings, autoCleanupDays: Number(e.target.value)})} className="flex-1" />
              <span className="text-sm font-black text-blue-600">{sysSettings.autoCleanupDays} 天</span>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-between items-center">
          <div>
            <h3 className="font-bold text-slate-800">暫停接收列印</h3>
            <p className="text-xs text-slate-400">目前狀態: {sysSettings.isPaused ? '🚫 已暫停接收' : '✅ 正常運作中'}</p>
          </div>
          <button onClick={() => updateSettings({...sysSettings, isPaused: !sysSettings.isPaused})} className={`w-14 h-7 rounded-full relative transition-colors ${sysSettings.isPaused ? 'bg-rose-500' : 'bg-slate-200'}`}>
            <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all ${sysSettings.isPaused ? 'left-8' : 'left-1'}`}></div>
          </button>
        </div>
  ...
  const BottomNav = () => {
    const tabs = isAdminView ? [
      ...
      { id: 'admin_settings', icon: '⚙️', label: '設定', action: fetchSettings }
    ] : [
    ...
    return (
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-sm h-[72px] bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-white/40 shadow-[0_8px_30px_rgb(0,0,0,0.08)] flex justify-around items-center px-2 z-50">
        {tabs.map(t => (
          <button key={t.id} onClick={() => { setActiveTab(t.id); if(t.action) t.action(); }} className={`relative flex flex-col items-center justify-center ${isAdminView ? 'w-[56px]' : 'w-[72px]'} h-[56px] rounded-[1.5rem] transition-all duration-300 ${activeTab === t.id ? 'bg-[#0f172a] text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
